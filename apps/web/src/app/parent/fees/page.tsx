...
interface StudentFee {
  id: string; month: number | null; year: number; amount: number; paidAmount: number; remainingAmount: number; dueDate: string; status: string; feeType: string | null;
}
interface ExtraFee { id: string; title: string; amount: number; paidAmount: number; remainingAmount: number; dueDate: string; status: string }
interface FeesResponse { monthlyFees: StudentFee[]; extraFees: ExtraFee[] }
interface Payment { id: string; studentId: string; amount: number; status: string; paidAt: string | null; method: string | null; gateway: string; createdAt: string }
interface ReceiptData {
  school: { name: string; address: string | null; phone: string | null; email: string | null };
  receiptNumber: string | null; student: { name: string; admissionNumber: string } | null;
  class: string | null; section: string | null; feeType: string | null; feeMonth: number | null;
  amount: number; remainingAfterThisPayment: number | null; paymentCategory: string; method: string | null;
  transactionId: string | null; paymentDate: string | null; status: string; paymentId: string;
}
...
export default function ParentFeesPage() {
  ...
  const load = useCallback(() => {
    ...
    api.get<Payment[]>("/payments/mine").then((all) => setPayments(all.filter((p) => p.studentId === selectedChildId)));
  }, [selectedChildId]);
  ...
  async function handlePay(kind: "monthly" | "extra", fee: StudentFee | ExtraFee) {
    ...
    await payFee({
      ...
      onSuccess: () => {
        setMessage({ type: "success", text: "Payment successful! The receipt is now available below." });
        setPayingId(null);
        load();
      },
      onError: (msg) => {
    ...
    <PortalShell navItems={PARENT_NAV} loginPath="/parent/login" allowedRoles={["PARENT"]} portalLabel="Parent Portal">
      ...
      <div className="space-y-8 p-6 sm:p-8">
        ...
        {fees && (
          <section>
            ...
            {pendingMonthly.length === 0 && pendingExtra.length === 0 ? (
              <p className="rounded-lg border border-neutral-200 bg-white p-6 text-sm text-neutral-500">No pending fees for this child.</p>
            ) : (
              <div className="space-y-2">
                ...
                {pendingExtra.map((f) => (
                  <FeeRow key={f.id} title={f.title} amount={f.remainingAmount} paidAmount={f.paidAmount} dueDate={f.dueDate} status={f.status} onPay={() => handlePay("extra", f)} paying={payingId === f.id} />
                ))}
              </div>
            )}
          </section>
        ...
        <section>
          ...
          {payments && payments.length === 0 && <p className="text-sm text-neutral-500">No payment history yet for this child.</p>}
...
function FeeRow({ title, amount, paidAmount, dueDate, status, onPay, paying }: { title: string; amount: number; paidAmount: number; dueDate: string; status: string; onPay: () => void; paying: boolean }) {
  return (
    <div className="flex flex-col gap-3 rounded-lg border border-neutral-200 bg-white p-4 shadow-sm sm:flex-row sm:items-center sm:justify-between">
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="font-display font-bold text-navy">{formatCurrency(amount)}</span>

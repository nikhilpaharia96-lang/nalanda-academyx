...
export class TeachersService {
  ...
  async create(
    ...
    // The existing convention here is "EMP-" (not "TCH-") — kept as-is per
    // the instruction to reuse an existing, already-working ID convention
    ...
    // pattern rather than a real DB transaction.
    const [user] = await db
      .insert(schema.users)
  ...
  async update(id: string, dto: Partial<{ name: string; subject: string; department: string; qualification: string; designation: string; phone: string; address: string; status: string }>, actorId: string) {

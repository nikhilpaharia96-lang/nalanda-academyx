...
export class StudentsService {
  ...
  async create(dto: CreateStudentDto, actingUserId: string) {
    ...
    // No real cross-dialect DB transaction here (better-sqlite3's driver
    // rejects async transaction callbacks in this dual-dialect setup) —
    // instead, if the second insert fails, the just-created user row is
    // explicitly deleted before re-throwing, so no orphaned login account
    // is ever left behind.
    const [user] = await db
      .insert(schema.users)

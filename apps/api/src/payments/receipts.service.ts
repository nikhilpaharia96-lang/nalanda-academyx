...
export class ReceiptsService {
  ...
  async getFullReceipt(paymentId: string) {
    ...
    if (payment.studentFeeId) {
      ...
      feeType = "School Store Order";
    }

    let storeOrder: { orderNumber: string; items: Array<{ productName: string; variantLabel: string | null; quantity: number; unitPrice: number; lineTotal: number }> } | null = null;
    if (payment.storeOrderId) {
      const [order] = await db.select().from(schema.storeOrders).where(eq(schema.storeOrders.id, payment.storeOrderId));
      if (order) {
        const items = await db.select().from(schema.storeOrderItems).where(eq(schema.storeOrderItems.orderId, order.id));
        storeOrder = {
          orderNumber: order.orderNumber,
          items: items.map((i) => ({ productName: i.productName, variantLabel: i.variantLabel, quantity: i.quantity, unitPrice: i.unitPrice, lineTotal: i.lineTotal })),
        };
      }
    }

...
export class PaymentsService {
  ...
  private async resolveContext(dto: { studentFeeId?: string; extraFeeId?: string; admissionApplicationId?: string; storeOrderId?: string }) {
    if (dto.studentFeeId) {
    ...
    if (dto.storeOrderId) {
      const [order] = await db.select().from(schema.storeOrders).where(eq(schema.storeOrders.id, dto.storeOrderId));
      ...
      // Store orders have no partial-payment concept (unlike fees) — the
      // "remaining amount" is always simply the order's full total.
      return { studentId: order.studentId, paymentType: "STORE_ORDER" as const, remainingAmount: order.totalAmount, record: order };
    }
  ...
  async createRazorpayOrder(dto: CreateRazorpayOrderDto, user: AuthenticatedUser) {
    ...
    if (ctx.paymentType === "STORE_ORDER" && ctx.remainingAmount !== undefined && Math.abs(dto.amount - ctx.remainingAmount) > AMOUNT_EPSILON) {
      throw new BadRequestException(`School Store orders must be paid in full (₹${ctx.remainingAmount.toFixed(2)})`);
    }
  ...
  private async allocatePaymentToFee(payment: typeof schema.payments.$inferSelect) {
    ...
    if (payment.storeOrderId) {
      await this.storeOrdersService.confirmPaid(payment.storeOrderId, { paymentId: payment.id });
    }
  }
